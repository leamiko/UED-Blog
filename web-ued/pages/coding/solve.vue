<template>
  <div>
    <div class="my-content cus-box-center">
      <el-card>
        ...
        <div class="card-content">
          <el-avatar :size="48"
                     :src="avator"
                     class="align-top"></el-avatar>
          <div class="inline inline_auto">
            <el-input v-model="input"
                      :placeholder="'请一句话描述问题'"
                  ...
                  <div class="my-menu-content">
                    <ul>
                      <li class="pointer"
                          v-for="item in list"
                          :key="item.id"
                          @click="descList = item.children">{{item.name}}</li>
                    </ul>
                    <div>
                      <my-tag class="my-tag pointer"
                              :class="{'active': checkList.indexOf(item) >= 0}"
                              v-for="item in descList"
                              :key="'desc' + item._id"
                              :text="item.name"
                              @active="checkLabel(item)"></my-tag>
                    </div>
                  </div>
                </el-dropdown-menu>
              </el-dropdown>
            ...
            <my-editor :height="'374px'"
                       :placeholder="'可以详细描述问题，也可以插入图片和代码块（选填）'"
                       @change="onEditor1Change"
                       ref="myEditor1"></my-editor><br>
            <my-editor :height="'374px'"
                       :placeholder="'请将您的解决方案写在此处，感谢你为其他码农作出的贡献！'"
                       @change="onEditor2Change"
                       ref="myEditor2"></my-editor><br>
            <div class="text-right">
              <el-checkbox v-model="isAnonymous">匿名只是你穿的保护色～</el-checkbox>&emsp;&emsp;
              <el-button type="primary"
                         round
                         size="small"
                         @click="submit()">&emsp;发&nbsp;布&emsp;</el-button>
            </div>
          </div>
        </div>
      </el-card>
    ...
    <el-dialog :visible.sync="showDialog"
               ...
               center
               append-to-body
               :close-on-click-modal="resultError"
               :close-on-press-escape="resultError">
      <template slot="title">
